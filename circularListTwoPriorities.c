    ...
    if (txtFile == NULL) {
        ...
        while (get_one_line(txtFile, fileLine, MAXLINESIZE)) {
            ...
            //to fill in the parameters test2 and mean to be filled
                ...
                checkTests(token, error);
                if (printError(error, lineNumber, &errors, txtFileName) == 1) {
                    printf("nextElem Name: %s, nextElem Subject: %s\n", nextElem->name, nextElem->schoolSubject);
                    free(nextElem);
                    printf("nextElem Name: %s, nextElem Subject: %s\n", nextElem->name, nextElem->schoolSubject);
                    continue;
                }
...
void circularListOnePriority(struct studentNameNumber * newNode){
    int result = 0;
    struct studentNameNumber* nextElem, *currentElem;

    printf("\nINSERT\n");
    //If it is the first element of the list it starts off as the head
    if(nameNumberHead == NULL){
        printf("PRIMEIRA\n");
        nameNumberHead = newNode;
        newNode -> next = nameNumberHead;
    }else{
        //Inserts node at the beginning of the list
        if(strcasecmp(nameNumberHead -> studentName, newNode -> studentName) > 0){
            newNode -> next = nameNumberHead;

            //If there is only one element on the list we connect the previous head to the new head
            //directly
            if(nameNumberHead -> next == nameNumberHead){
                nameNumberHead -> next = newNode;
            }
            ...
            currentElem = nameNumberHead;
            nextElem = nameNumberHead -> next;
            while((result = strcasecmp(nextElem -> studentName, newNode -> studentName)) < 0
                  && nextElem != nameNumberHead) {
                currentElem = nextElem;
                nextElem = nextElem->next;
            }

            printf("nextElem -> next: %s\n", nextElem -> studentName);
            if(nextElem->next == nameNumberHead && result < 0){
                printf("FIM LISTA\n");
...
int circularListTwoPriorities(struct elemInQueue * newNode){
    ...
    if(head == NULL){
        ...
        if(strcasecmp(head -> schoolSubject, newNode -> schoolSubject) > 0){
            ...
            printf("discipline: %s, name: %s, current Elem: %s, nextElem: %s\n",newNode->schoolSubject, newNode -> name, currentElem -> name, nextElem-> name);
